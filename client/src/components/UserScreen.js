...
const UserScreen = () => {
    ...
    let bottons3=
            <IconButton disabled="true" style={{color:"green"}} size='small'onClick={handleUser}><PersonIcon style={{fontSize: '24pt'}}/></IconButton>
    let bottons4=
            <IconButton size='small' onClick={handleSortOpen} style={{float:"right",color:"black"}}><SortIcon/></IconButton>
    let text1 = <Box position= "absolute" style={{transform:"translate(110%,-110%)",fontSize: '12pt'}}><input id="searchfield" style={{transform:"translate(-10%,0%)",width: '400px'}} className='modal-textfield' type="text" defaultValue= "Search..." onKeyPress={handleKeyPress}
    onChange={handleUpdateText} /></Box>;
    ...
    if(store.selectedList!=null&&!auth.loggedIn){
        if(store.selectedList.ispublish){
            commentBottom=<Button 
                disabled={!playerMode||store.selectedList==null||!auth.loggedIn}
